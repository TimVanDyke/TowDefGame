    ...
    B: BufferType,
{
    pub fn new(gl: &gl::Gl) -> Buffer<B> {
        let mut buffer_ptr: gl::types::GLuint = 0;
        unsafe {
            gl.GenBuffers(1, &mut buffer_ptr);
        }
    ...
    pub fn bind(&self) {
        unsafe {
            self.gl.BindBuffer(B::BUFFER_TYPE, self.buffer_ptr);
        }
    }
    ...
    B: BufferType,
{
    fn drop(&mut self) {
        unsafe {
            self.gl.DeleteBuffers(1, &self.buffer_ptr);
        }
    }
}
